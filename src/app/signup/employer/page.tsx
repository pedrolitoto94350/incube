...
export default function SignupEmployerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error: signUpError } = await supabase.auth.signUp({
      ...
      options: { data: { company_name: form.company_name, role: "employer" } },
    });
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold mt-6 mb-2">Inscription employeur</h1>
          <p className="text-gray-500 text-sm">Trouvez les talents de demain</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 border border-gray-100 space-y-5">
          ...
          <div>
            ...
            <input type="text" value={form.company_name} onChange={(e) => setForm({ ...form, company_name: e.target.value })} required className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-400 outline-none transition-all" />
          </div>
          <div>
